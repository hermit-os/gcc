...
echo "deb [trusted=yes] https://packagecloud.io/hermitcore/ubuntu vivid main" | tee -a /etc/apt/sources.list 
apt-get update
apt-get install -y --allow-unauthenticated binutils-hermit
